...
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" experiments/run_s2e_pipeline.py --data "${DATA}" --model-path "${MODEL_DIR}" --output "${OUTPUT_ROOT}/cpp_pretrain" --limit "${INSTANCE_LIMIT:-100}" 2>&1 | tee "${OUTPUT_ROOT}/logs/cpp_pretrain.log"
