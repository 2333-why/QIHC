...
python experiments/prepare_s2e_training_data.py --data "${DATA}" --records "${OUTPUT_ROOT}/cpp_pretrain/records.jsonl" --output "${OUTPUT_ROOT}/training_data"
torchrun --standalone --nproc_per_node=4 experiments/run_cvrp_lns.py --dataset jsonl --data "${DATA}" --output "${OUTPUT_ROOT}/solver_full" --limit "${INSTANCE_LIMIT:-100}" --search-seeds 0 1 2 3 4 --methods random knn llm --sampler torch --iterations "${LNS_ITERATIONS:-100}" --patience 30 --destroy-size 12 --routes-per-customer 4 --sampling-steps 1000 --num-chains 2048 --top-samples 32 --model-path "${MODEL_DIR}" 2>&1 | tee "${OUTPUT_ROOT}/logs/solver.log"
...
if [[ "${RUN_TRAINING:-1}" == "1" ]]; then
  ...
  if [[ -s "${OUTPUT_ROOT}/training_data/dpo.jsonl" ]]; then torchrun --standalone --nproc_per_node=4 experiments/train_s2e_lora.py --stage dpo --model-path "${MODEL_DIR}" --data "${OUTPUT_ROOT}/training_data/dpo.jsonl" --output "${OUTPUT_ROOT}/checkpoints/dpo" --max-steps "${DPO_STEPS:-300}" 2>&1 | tee "${OUTPUT_ROOT}/logs/dpo.log"; fi
  torchrun --standalone --nproc_per_node=4 experiments/train_s2e_lora.py --stage grpo --model-path "${MODEL_DIR}" --data "${OUTPUT_ROOT}/training_data/grpo.jsonl" --output "${OUTPUT_ROOT}/checkpoints/grpo" --max-steps "${GRPO_STEPS:-300}" 2>&1 | tee "${OUTPUT_ROOT}/logs/grpo.log"
fi
