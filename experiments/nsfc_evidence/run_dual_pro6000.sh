...
#
# 后台：
#   nohup bash experiments/nsfc_evidence/run_dual_pro6000.sh \
#     > experiments/outputs/nsfc_evidence/dual_pro6000_launcher.log 2>&1 &
#
# 环境变量：同 run_dual_4090.sh，另：
#   SKIP_CASE_H=0                 # 默认 0（Pro 6000 可跑 14B 对比）
...
RUN_DIR="${REPO_ROOT}/experiments/outputs/nsfc_evidence/dual_pro6000_${TS}"
...
if [[ "${SKIP_SETUP}" != "1" ]]; then
  ...
  if [[ "${SKIP_MODEL_DOWNLOAD}" != "1" ]] && [[ ! -f "${MODEL_LOCAL_DIR}/config.json" ]]; then
    ...
    echo "[skip] 7B 已存在: ${MODEL_LOCAL_DIR}"
    MODEL="${MODEL_LOCAL_DIR}"
  fi

  if [[ "${SKIP_CASE_H}" != "1" ]] && [[ ! -f "${MODEL_LOCAL_14B}/config.json" ]]; then
    echo "[$(date -Iseconds)] 预下载 14B → ${MODEL_LOCAL_14B} (Case H)..."
    mkdir -p "${MODEL_LOCAL_14B}"
    "${PYTHON}" experiments/nsfc_evidence/download_model_hf.py \
      --repo "${MODEL_NAME_14B}" \
      --local-dir "${MODEL_LOCAL_14B}" \
      --backend "${MODEL_DOWNLOAD_BACKEND:-auto}" || echo "WARN: 14B 下载失败，稍后 SKIP_CASE_H=1"
  fi
...
if [[ "${SKIP_CASE_H}" != "1" ]]; then
  ...
  if [[ ! -f "${MODEL_14B}/config.json" ]]; then
    MODEL_14B="${MODEL_NAME_14B}"
  fi
  echo "[$(date -Iseconds)] Case H 14B（Pro 6000 单卡）..."
  CUDA_VISIBLE_DEVICES=0 "${PYTHON}" experiments/nsfc_evidence/run_case_h_model_compare.py \
    ...
    2>&1 | tee "${RUN_DIR}/case_h.log" || echo "WARN: Case H 失败，可 SKIP_CASE_H=1 重跑"
fi
...
ARCHIVE="${RUN_DIR}/dual_pro6000_results.tar.gz"
tar -czf "${ARCHIVE}" -C "${RUN_DIR}" .

echo "=============================================="
echo " 2×Pro 6000 全量实验完成"
echo " 结果: ${RUN_DIR}"
echo " 打包: ${ARCHIVE}"
echo "----------------------------------------------"
